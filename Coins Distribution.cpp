/*Find the minimum number of coins required to form any value between 1 to N, both inclusive.
  ...
  Let's understand the problem using the following example. Consider the value of N is 13, then the minimum number of coins required to formulate any value between 1 and 13, is 6.
  One 5 Rupee, three 2 Rupee and two 1 Rupee coins are required to realize any value between 1 and 13. Hence this is the answer. However, if one takes two 5 Rupee coins,
  one 2 rupee coins and two 1 rupee coins, then to all values between 1 and 13 are achieved. But since the cumulative value of all coins equals 14, i.e., exceeds 13,
...
int main()
{
  int n, five, two, one;
  cin >> n;
  five = (n - 4) / 5;		 // No. of five rupee coins
  n = n - (five * 5);    // Excluding Five rupee coins from N
  if(n % 2 == 0) 		 // Checking the no. of 1 rupee coins
    one = 2;
  else
    one = 1;
  two = (n - one) / 2; 	 // No. of 2 rupee coins.
  cout << five+two+one << " " << five <<" " << two << " "<< one; // Bcoz of the output format
  return 0;
}
